...
const fileOperationsHandler = async (command, args) => {
  if (command === FileOperationsCommands.cat) {
    await catFile(args[0]);
  } else if (command === FileOperationsCommands.add) {
    await addFile(args[0]);
  } else if (command === FileOperationsCommands.mkdir) {
    await makeDir(args[0]);
  } else if (command === FileOperationsCommands.rn) {
    await renameFile(args[0], args[1]);
  } else if (command === FileOperationsCommands.cp) {
    await copyFile(args[0], args[1]);
  } else if (command === FileOperationsCommands.mv) {
    await moveFile(args[0], args[1]);
  } else if (command === FileOperationsCommands.rm) {
