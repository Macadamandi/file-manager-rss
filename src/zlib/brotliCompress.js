...
const brotliCompress = async (pathToFile, pathToDestination) => {
  try {
    ...
    if (destinationDirExists) {
      destinationPath = join(destinationPath, baseName + ".br");
    }
    ...
    await pipeline(createReadStream(sourcePath), createBrotliCompress(), createWriteStream(destinationPath));

    console.log(`File ${sourcePath} was successfully compressed to ${destinationPath}`);
